...
import type { AuthorFilterRequest, AuthorStatus } from "../types/author.type";

const initialFilterOptions = {
  keyword: "",
  status: null as AuthorStatus | null,
  page: 1,
  size: 10,
};
...
export const useAuthorFilter = () => {
  ...
  const [status, setStatus] = useState<AuthorStatus | null>(
    () => (searchParams.get("status") as AuthorStatus) ?? initialFilterOptions.status
  );
  ...
  const handleStatusChange = useCallback((value: AuthorStatus | null) => {
    setStatus(value);
    setPage(1);
  }, []);

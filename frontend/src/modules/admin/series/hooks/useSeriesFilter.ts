...
import type { SeriesFilterRequest, SeriesStatus } from "../types/series.type";

const initialFilterOptions = {
  keyword: "",
  status: null as SeriesStatus | null,
  page: 1,
  size: 10,
};
...
export const useSeriesFilter = () => {
  ...
  const [status, setStatus] = useState<SeriesStatus | null>(
    () => (searchParams.get("status") as SeriesStatus) ?? initialFilterOptions.status
  );
  ...
  const handleStatusChange = useCallback((value: SeriesStatus | null) => {
    setStatus(value);
    setPage(1);
  }, []);
  ...
  const filterParams: SeriesFilterRequest = {

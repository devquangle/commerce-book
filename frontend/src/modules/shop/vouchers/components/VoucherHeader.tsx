...
export type VoucherHeaderMode = "list" | "add" | "update";

export interface VoucherHeaderProps {
  mode?: VoucherHeaderMode;
  isEdit?: boolean;
  title?: string;
  subTitle?: string;
  submitText?: string;
...
export const VoucherHeader: React.FC<VoucherHeaderProps> = ({
  ...
  const actualMode: VoucherHeaderMode =
    mode || (isEdit ? "update" : "list");

  // Mode: List
  if (actualMode === "list") {
    const displayTitle = title || "Voucher";
    const displaySubTitle =
      subTitle || "Quản lý danh sách voucher của cửa hàng";
  ...
  const displayTitle =
    title || (isUpdateMode ? "Cập nhật thông tin voucher" : "Thêm voucher mới");
  const displaySubTitle =
    subTitle ||
    (isUpdateMode
      ? "Chỉnh sửa các thông tin voucher trong hệ thống."
      : "Điền đầy đủ thông tin bên dưới để khởi tạo voucher mới trong hệ thống.");

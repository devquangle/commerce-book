...
import type { PromotionFilterRequest, PromotionStatus } from "../types/promotion.type";
...
export const usePromotionShopFilter = () => {
  ...
  const [status, setStatus] = useState<PromotionStatus | null>(
    () => (searchParams.get("status") as PromotionStatus) ?? initialFilterOptions.status
  );
  ...
  const handleStatusChange = useCallback((value: PromotionStatus | null) => {
    setStatus(value);
    setPage(1);
  }, []);
  ...
  const filterParams: PromotionFilterRequest = {

...
const ProductService = {
  ...
  approve: async (id: number): Promise<void> => {
    ...
    if (!response.data.success) {
      throw new Error(response.data.message || "Approve product failed");
    }
  },

  reject: async (id: number, reason: string): Promise<void> => {
    const response = await authAxios.put<ApiResponse<void>>(
      `/api/v1/admin/products/reject/${id}`,
      { reason },
    );
    if (!response.data.success) {
      throw new Error(response.data.message || "Reject product failed");
    }
  },
};

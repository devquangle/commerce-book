...
import type { GenreStatus } from "@/modules/admin/genres/types/genre.type";
import { SelectBox } from "@/components/ui/SelectBox";
import { Button } from "@/components/ui/Button";

interface GenreFilterProps {
  keyword: string;
  status: GenreStatus | null;
  onKeywordChange: (keyword: string) => void;
  onStatusChange: (status: GenreStatus | null) => void;
  onReset: () => void;
}
    ...
    <div className="card-custom flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2 min-w-50">
          ...
          <SelectBox
            ...
            value={status || ""}
            onChange={(e) =>
              onStatusChange(e.target.value ? (e.target.value as GenreStatus) : null)
            }

...
} from "./user-sidebar-menu";

export const UserSidebar: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const { pathname } = useLocation();

  const isMenuItemActive = (item: UserSidebarMenuItem): boolean => {
    if (item.href) {
      if (item.href === "/") {
        return pathname === "/";
      }
  ...
  const activeParent = userSidebarMenu.find(
  ...
  return (
    <>
      ...
      <aside
        id="top-bar-sidebar"
        className={`fixed px-2 top-0 left-0 z-40 w-64 h-full pt-17 transition-transform duration-300 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } sm:hidden bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800`}
        aria-label="Sidebar"
      >
